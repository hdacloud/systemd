...
you want to make sure that this patch works cleanly with the systemd version in
the systemd.spec (ex: v257.2), and to the HEAD of systemd upstream
(systemd-257~devel-20250303020517).
...
Pipeline schedules). The pipeline builds only HEAD version of systemd: one
per `REPO`, `RELEASE` (ex: systemd-258~devel-20250303020517.hs.el10). It
doesn't perform tests. It publishes the RPMs using the 'testing' tag, ex,
